...
import {allRegions } from '../../hooks/region';
...
import { getDistrict, saveDistrict } from '../../hooks/district';
...
class Index extends Component {
    ...
    constructor(props){
        super(props);
        this.state = { block: false, id:null, name: '', shipping: 0, region: null, regions : []};
    ...
    async componentDidMount(){
        ...
        if(itemId !=null){
            ...
            var {response} = await getDistrict(itemId, fields)
            if(response?.__typename == 'District'){
                this.setState({ id: response.id, name: response.name, shipping: response.shipping, region: response.region, block:false })
            }
    ...
    getParents = async() => {
        const regionFields = {"regionName": true}
        var {response} = await allRegions(regionFields)
        if(response){
            this.setState({regions: response.regions})
        }
    }
    ...
    checkInput = (e) => {
        ...
        e.preventDefault()
        const {name, shipping, region} = this.state
        var errorMessage
    ...
    saveItem = async () => {
        ...
        const {id, name, shipping, region} = this.state

        var {response } = await saveDistrict(id, name, shipping, region?.id)
                            ...
                            <form className='w-full h-full' onSubmit={(e) => this.checkInput(e)}>
                                ...
                                <div className='w-full h-full pt-4'>
                                    ...
                                    <div className='flex flex-row justify-between px-5'>
                                        ...
                                        <div className='text-lg font-semibold text-purple-600'>Formulaire Zones de livraisons</div>
                                    ...
                                    <div className='app-form overflow-y-auto mt-3 px-5'>
                                        ...
                                        <div className='col-span-2 px-4 py-4 '>
                                            <div className='flex flex-row'>
                                                ...
                                                <div className='self-center text-xl font-semibold text-gray-900 '>Zones de livraison</div>
                                            </div>
                                        ...
                                        <div className='col-span-3 bg-gray-200 bg-opacity-60 rounded-xl px-5 py-6'>
                                            ...
                                            <div className="w-full mb-5">
                                                <label htmlFor="name" className="block text-sm font-medium text-gray-900">Prix de livraison <span className='font-bold text-purple-600'>*</span> </label>
                                                <input type="number" value={this.state.shipping} onChange={(e) => this.setState({shipping:e.target.value }) } name="name" id="name" autoComplete="title" placeholder="Désignation" className="mt-1 h-10 w-full shadow-sm text-sm border border-gray-400 focus:border-0 focus:ring-2 focus:ring-purple-500 shadow-inner bg-white bg-opacity-90 rounded-md px-2" required/>
                                                <p className="mt-2 text-xs text-gray-600">ⓘ Concerne seulement les commandes à livrer dans cette zone.</p>
                                            </div>

                                            <div className='w-full mb-1 '>
                                                <Listbox value={region} onChange={(e) => this.setState({region: e})}>
                                                    {({ open }) => (
                                                        <>
                                                        <Listbox.Label className="block text-sm font-medium text-gray-900">Choisir une région <span className='font-bold text-purple-600'>*</span></Listbox.Label>
                                                        <div className="mt-1 relative">
                                                            <Listbox.Button className="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 sm:text-sm">
                                                                <span className="flex items-center">
                                                                    <span className="ml-3 block truncate">{region ? region.name : 'Choisir une région'}</span>
                                                                </span>
                                                                    ...
                                                                    <Listbox.Option key={item.id} className={({ active }) => classNames(active ? 'text-white bg-purple-600' : 'text-gray-900','cursor-pointer select-none relative py-2 pl-3 pr-9')} value={item}>
                                                                        ...
                                                                        <>
                                                                            <div className="flex items-center">
                                                                                <span className={classNames(region ? 'font-semibold' : 'font-normal', 'ml-3 block truncate')}>
                                                                                    {item.name}
                                                                                </span>
                                                                            </div>
